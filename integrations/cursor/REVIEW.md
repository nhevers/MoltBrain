...
3. **Parity with Claude Code**: Matches claude-recall's hook behavior where possible
...
**Purpose**: Initialize claude-recall session when prompt is submitted
...
- ✅ Observation format matches claude-recall structure
...
**How It Works**:
1. Fetches context from claude-recall worker
2. Writes to `.cursor/rules/claude-recall-context.mdc` with `alwaysApply: true`
...
6. Write context to `.cursor/rules/claude-recall-context.mdc`
...
3. **Context Injection**:
   - ✅ Solved via auto-updated `.cursor/rules/claude-recall-context.mdc`
