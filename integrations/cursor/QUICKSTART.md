# Quick Start: claude-recall + Cursor Integration

> **Give your Cursor AI persistent memory in under 5 minutes**

## What This Does

Connects claude-recall to Cursor so that:
...
claude-recall settings set CLAUDE_RECALL_PROVIDER gemini
claude-recall settings set CLAUDE_RECALL_GEMINI_API_KEY your-api-key

# Option B: OpenRouter (free models available)
claude-recall settings set CLAUDE_RECALL_PROVIDER openrouter
claude-recall settings set CLAUDE_RECALL_OPENROUTER_API_KEY your-api-key
```
...
- claude-recall provides search tools via MCP
...
- Check logs: `tail -f ~/.claude-recall/logs/worker-$(date +%Y-%m-%d).log`
...
- Visit [claude-recall docs](https://docs.claude-recall.ai) for full feature set
