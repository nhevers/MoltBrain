...
1. **Install**: `claude-recall cursor install` creates initial context file
2. **Stop hook**: `session-summary.sh` updates context after each session ends
3. **Cursor**: Automatically includes `.cursor/rules/claude-recall-context.mdc` in all chats
...
2. Creates `lifecycle.json` configuration
3. Fetches existing context from claude-recall and writes to `.cursor/rules/claude-recall-context.mdc`
...
cat > .cursor/rules/claude-recall-context.mdc << EOF
---
alwaysApply: true
---
# Memory Context
...
Located at: `.cursor/rules/claude-recall-context.mdc`

```markdown
---
alwaysApply: true
description: "claude-recall context from past sessions (auto-updated)"
---

# Memory Context from Past Sessions

[Your context from claude-recall appears here]
...
When you run `claude-recall cursor install`, the project is registered in `~/.claude-recall/cursor-projects.json`. This allows the worker to automatically update your context file whenever a new summary is generated - even if it happens from Claude Code or another IDE working on the same project.

To see registered projects:
```bash
cat ~/.claude-recall/cursor-projects.json
```
...
| Initial setup | Automatic | `claude-recall cursor install` creates initial context |
| MCP tool access | ✅ Full support | ✅ Full support |
| Web viewer | ✅ Available | ✅ Available |

## First Session Behavior

When you run `claude-recall cursor install`:
...
Configure claude-recall's MCP server in Cursor for search tools:
...
Ask the agent: "Check claude-recall for any previous work on authentication"

## File Location

The context file is created at:
```
<workspace>/.cursor/rules/claude-recall-context.mdc
```

This is version-controlled by default. Add to `.gitignore` if you don't want to commit it:
```
.cursor/rules/claude-recall-context.mdc
```
