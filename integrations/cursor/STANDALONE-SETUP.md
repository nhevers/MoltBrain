# claude-recall for Cursor (No Claude Code Required)

> **Persistent AI Memory for Cursor - Zero Cost to Start**

## Overview

Use claude-recall's persistent memory in Cursor without a Claude Code subscription. Choose between free-tier providers (Gemini, OpenRouter) or paid options.
...
**Why This Matters**: Every Cursor session starts fresh. claude-recall bridges that gap - your AI agent builds cumulative knowledge about your codebase, decisions, and patterns over time.
...
Since you don't have Claude Code, you need to configure an AI provider for claude-recall's summarization engine.
...
cat > ~/.claude-recall/settings.json << 'EOF'
{
  "CLAUDE_RECALL_PROVIDER": "gemini",
  "CLAUDE_RECALL_GEMINI_API_KEY": "YOUR_GEMINI_API_KEY",
...
cat > ~/.claude-recall/settings.json << 'EOF'
{
  "CLAUDE_RECALL_PROVIDER": "openrouter",
  "CLAUDE_RECALL_OPENROUTER_API_KEY": "YOUR_OPENROUTER_API_KEY"
}
EOF
```
...
```bash
# From the claude-recall repo directory (recommended - all projects)
...
- Hook configuration to `.cursor/lifecycle.json`
...
```bash
cat ~/.claude-recall/settings.json
```
...
- Visit https://docs.claude-recall.ai for full documentation
...
$settingsDir = "$env:USERPROFILE\.claude-recall"
...
The `lifecycle.json` file is configured to invoke PowerShell with `-ExecutionPolicy Bypass` to ensure scripts run without additional configuration.
...
Verify the lifecycle.json contains PowerShell invocations:
```powershell
Get-Content .cursor\lifecycle.json
```
