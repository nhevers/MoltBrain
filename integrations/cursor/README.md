# claude-recall Cursor Hooks Integration
...
cd claude-recall && bun install && bun run build
...
The hooks bridge Cursor's hook system to claude-recall's worker API, allowing:
...
1. **Install**: `claude-recall cursor install` generates initial context
2. **Stop hook**: Updates context in `.cursor/rules/claude-recall-context.mdc` after each session
...
- **MCP Tools**: Configure claude-recall's MCP server for `search`, `timeline`, `get_observations` tools
...
claude-recall cursor install user

# Or install for current project only
claude-recall cursor install
```
...
```bash
# Copy lifecycle.json to your home directory
cp integrations/cursor/lifecycle.json ~/.cursor/lifecycle.json

# Copy hook scripts
mkdir -p ~/.cursor/hooks
cp integrations/cursor/*.sh ~/.cursor/hooks/
chmod +x ~/.cursor/hooks/*.sh
```

**Project-level** (for per-project hooks):
```bash
# Copy lifecycle.json to your project
mkdir -p .cursor
cp integrations/cursor/lifecycle.json .cursor/lifecycle.json
...
|-------------|--------|---------|
| `beforeSubmitPrompt` | `session-init.sh` | Initialize claude-recall session |
...
- Initializes a new session in claude-recall using `conversation_id` as the session ID
- Extracts project name from workspace root
- Outputs `{"continue": true}` to allow prompt submission

### Context Hook (`context-inject.sh`)
- Ensures claude-recall worker is running before session
- Outputs `{"continue": true}` to allow prompt submission
...
- Maps them to claude-recall's observation format
...
- Requests summary generation from claude-recall
- **Updates context file** in `.cursor/rules/claude-recall-context.mdc` for next session

## Configuration

The hooks read configuration from `~/.claude-recall/settings.json`:

- `CLAUDE_RECALL_WORKER_PORT`: Worker port (default: 37777)
- `CLAUDE_RECALL_WORKER_HOST`: Worker host (default: 127.0.0.1)
...
1. Check hooks are in the correct location:
   ```bash
   ls .cursor/lifecycle.json  # Project-level
   ls ~/.cursor/lifecycle.json  # User-level
   ```
...
- `lifecycle.json` - Hook configuration
...
- [claude-recall Documentation](https://docs.claude-recall.ai)
- [Cursor Hooks Reference](../docs/context/cursor-hooks-reference.md)
- [claude-recall Architecture](https://docs.claude-recall.ai/architecture/overview)
