# claude-recall ↔ Cursor Integration Architecture

## Overview

This integration connects claude-recall's persistent memory system to Cursor's hook system, enabling:
...
session-init.sh
    ...
    └─ Initialize session in claude-recall
    ↓
context-inject.sh
...
save-observation.sh
    ...
    ├─ Map to claude-recall observation format
...
| Cursor Field | claude-recall Field | Notes |
|-------------|------------------|-------|
...
| Cursor Event | claude-recall Tool Name | Input Format |
|-------------|---------------------|--------------|
...
### Worker Settings
Located in `~/.claude-recall/settings.json`:
- `CLAUDE_RECALL_WORKER_PORT` (default: 37777)
- `CLAUDE_RECALL_WORKER_HOST` (default: 127.0.0.1)

### Hook Settings
Located in `lifecycle.json`:
- Hook event names
...
1. **Context Injection**: Cursor's `beforeSubmitPrompt` doesn't support prompt modification. Context must be retrieved via:
   - MCP tools (claude-recall provides search tools)
