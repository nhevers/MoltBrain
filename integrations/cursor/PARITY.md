# Feature Parity: claude-recall Hooks vs Cursor Hooks

This document compares claude-recall's Claude Code hooks with the Cursor hooks implementation to ensure feature parity.
...
**How it works**:
- Hook writes context to `.cursor/rules/claude-recall-context.mdc`
...
### Tag Stripping
- **Claude Code**: Worker handles `<private>` and `<claude-recall-context>` tags
...
**Overall**: The Cursor hooks implementation achieves **full functional parity** with claude-recall's Claude Code hooks:
