...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
claude-recall Pending Queue Manager
...
What is this for?
  If the claude-recall worker crashes or restarts, pending observations may
...
`);
  ...
  console.log('\n=== claude-recall Pending Queue Status ===\n');

  // Check worker health
  const healthy = await checkWorkerHealth();
  if (!healthy) {
    console.log('Worker is not running. Start it with:');
    console.log('  cd ~/.claude/plugins/marketplaces/nhevers && npm run worker:start\n');
    process.exit(1);
  }
