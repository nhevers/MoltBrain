#!/usr/bin/env node
/**
 * Build Windows executable for claude-recall worker service
...
try {
  execSync(
    `bun build --compile --minify --target=bun-windows-x64 ./src/core/worker-service.ts --outfile ${outDir}/worker-service-v${version}-win-x64.exe`,
    { stdio: 'inherit' }
  );
