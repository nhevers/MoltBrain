...
function stripProjectRoot(filePath: string): string {
  ...
  const markers = ['/Scripts/', '/packages/', '/runtime/', '/docs/'];
  ...
  const projectIndex = filePath.indexOf('claude-recall/');
  if (projectIndex !== -1) {
    return filePath.substring(projectIndex + 'claude-recall/'.length);
  }
