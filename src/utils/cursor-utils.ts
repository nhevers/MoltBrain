...
export function writeContextFile(workspacePath: string, context: string): void {
  ...
  const rulesFile = join(rulesDir, 'claude-recall-context.mdc');
...
The following context is from claude-recall, a persistent memory system that tracks your coding sessions.

${context}

---
*Updated after last session. Use claude-recall's MCP search tools for more detailed queries.*
`;
...
export function readContextFile(workspacePath: string): string | null {
  const rulesFile = join(workspacePath, '.cursor', 'rules', 'claude-recall-context.mdc');
...
/**
 * Configure claude-recall MCP server in Cursor's mcp.json
...
/**
 * Remove claude-recall MCP server from Cursor's mcp.json
...
export function removeMcpConfig(mcpJsonPath: string): void {
  ...
  try {
    ...
    if (config.mcpServers && config.mcpServers['claude-recall']) {
      delete config.mcpServers['claude-recall'];
      writeFileSync(mcpJsonPath, JSON.stringify(config, null, 2));
    }
  } catch (e) {
