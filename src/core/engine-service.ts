 ...
 * - src/core/server/ - HTTP server, middleware, error handling
 * - src/core/infrastructure/ - Process management, health monitoring, shutdown
 * - src/core/worker/ - Business logic, routes, agents
 */

import path from 'path';
import { Client } from '@modelcontextprotocol/sdk/client/index.js';
import { StdioClientTransport } from '@modelcontextprotocol/sdk/client/stdio.js';
import { getWorkerPort, getWorkerHost } from '../common/engine-utils.js';
...
import {
  ...
  httpShutdown,
  checkVersionMatch
} from './infra/HealthMonitor.js';
import { performGracefulShutdown } from './infra/GracefulShutdown.js';
...
import { SSEBroadcaster } from './engine/SSEBroadcaster.js';
import { SDKAgent } from './engine/SDKAgent.js';
import { GeminiAgent } from './engine/GeminiAgent.js';
import { OpenRouterAgent } from './engine/OpenRouterAgent.js';
import { PaginationHelper } from './engine/PaginationHelper.js';
import { SettingsManager } from './engine/SettingsManager.js';
import { SearchManager } from './engine/SearchManager.js';
import { FormattingService } from './engine/FormattingService.js';
import { TimelineService } from './engine/TimelineService.js';
import { SessionEventBroadcaster } from './engine/events/SessionEventBroadcaster.js';

// HTTP route handlers
import { ViewerRoutes } from './engine/http/routes/ViewerRoutes.js';
import { SessionRoutes } from './engine/http/routes/SessionRoutes.js';
import { DataRoutes } from './engine/http/routes/DataRoutes.js';
import { SearchRoutes } from './engine/http/routes/SearchRoutes.js';
import { SettingsRoutes } from './engine/http/routes/SettingsRoutes.js';
import { LogsRoutes } from './engine/http/routes/LogsRoutes.js';

// Process management for zombie cleanup (Issue #737)
import { startOrphanReaper, reapOrphanedProcesses } from './engine/ProcessRegistry.js';
...
export class WorkerService {
  ...
  private async initializeBackground(): Promise<void> {
    try {
      ...
      const { PendingMessageStore } = await import('./storage/PendingMessageStore.js');
      ...
      const mcpServerPath = path.join(__dirname, 'search-server.cjs');
...
async function main() {
  ...
  switch (command) {
    ...
    case 'hook': {
      ...
      if (!platform || !event) {
        console.error('Usage: claude-recall hook <platform> <event>');
        console.error('Platforms: claude-code, raw');
        console.error('Events: context, session-init, observation, summarize, user-message');
        process.exit(1);
      }
      const { hookCommand } = await import('../interface/hook-command.js');
