...
import { ParsedObservation, ParsedSummary } from '../../parser/parser.js';
...
export class ChromaSync {
  ...
  // See: https://github.com/anthropics/claude-recall/issues/675
  ...
  constructor(project: string) {
    ...
    this.VECTOR_DB_DIR = path.join(os.homedir(), '.claude-recall', 'vector-db');
  ...
  private async ensureConnection(): Promise<void> {
    ...
    try {
      ...
      // See: https://github.com/nhevers/claude-recall/issues/170 (Python 3.14 incompatibility)
      const settings = SettingsDefaultsManager.loadFromFile(USER_SETTINGS_PATH);
      const pythonVersion = settings.CLAUDE_RECALL_PYTHON_VERSION;
      ...
      this.client = new Client({
        name: 'claude-recall-chroma-sync',
