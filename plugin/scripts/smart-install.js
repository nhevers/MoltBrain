#!/usr/bin/env node
/**
 * Smart Install Script for claude-recall
 *
...
const ROOT = join(homedir(), '.claude', 'plugins', 'marketplaces', 'nhevers');
...
/**
 * Add shell alias for claude-recall command
 */
function installCLI() {
  const WORKER_CLI = join(ROOT, 'plugin', 'scripts', 'engine-runtime.cjs');
  const bunPath = getBunPath() || 'bun';
  const aliasLine = `alias claude-recall='${bunPath} "${WORKER_CLI}"'`;
  ...
  try {
    if (IS_WINDOWS) {
      ...
      const functionDef = `function claude-recall { & "${bunPath}" "${WORKER_CLI}" $args }\n`;
      ...
      if (!existingContent.includes('function claude-recall')) {
        writeFileSync(profilePath, existingContent + '\n' + functionDef);
        console.error(`✅ PowerShell function added to profile`);
        console.error('   Restart your terminal to use: claude-recall <command>');
      }
    } else {
      ...
      for (const config of shellConfigs) {
        if (existsSync(config)) {
          ...
          if (!content.includes('alias claude-recall=')) {
...
try {
  ...
  if (needsInstall()) {
    ...
    const port = process.env.CLAUDE_RECALL_WORKER_PORT || 37777;
    console.error(`[claude-recall] Plugin updated to v${newVersion} - restarting worker...`);
    try {
    ...
    // Worker will be started fresh by next hook in chain (engine-runtime.cjs start)
  }
